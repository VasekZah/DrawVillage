import { state, CONFIG, ui, gameCanvas } from './config.js';
import { Building, Humanoid, WorldObject } from './classes.js';

// --- HELPER FUNCTIONS ---
export const worldToGrid = (x, y) => state.grid[Math.floor(y / CONFIG.GRID_SIZE)]?.[Math.floor(x / CONFIG.GRID_SIZE)] || null;
export const screenToWorld = (sx, sy) => ({ x: (sx - gameCanvas.width / 2) / state.camera.zoom + state.camera.x, y: (sy - gameCanvas.height / 2) / state.camera.zoom + state.camera.y });
...
export function updateGridForObject(obj, isAdding) {
    ...
    for (let y = startY; y < endY; y++) for (let x = startX; x < endX; x++) {
        if (state.grid[y]?.[x]) state.grid[y][x].walkable = !isAdding;
    }
}
export function calculateAccessPoints(building) {
    ...
    for (let y = startGridY; y < endGridY; y++) {
        for (let x = startGridX; x < endGridX; x++) {
            if (state.grid[y]?.[x]) {
                buildingNodes.add(state.grid[y][x]);
            }
        }
    }
...
export function addEntity(entity) {
    ...
    if (entity instanceof Humanoid) state.settlers.push(entity);
    else if (entity instanceof Building) { state.buildings.push(entity); if (entity.status !== 'blueprint') updateGridForObject(entity, true); }
    else if (entity instanceof WorldObject) state.worldObjects.push(entity);
}
export function removeEntity(entity) {
    state.entities = state.entities.filter(e => e.id !== entity.id);
    if (entity instanceof Humanoid) state.settlers = state.settlers.filter(e => e.id !== entity.id);
    else if (entity instanceof Building) { state.buildings = state.buildings.filter(e => e.id !== entity.id); updateGridForObject(entity, false); }
    else if (entity instanceof WorldObject) state.worldObjects = state.worldObjects.filter(e => e.id !== entity.id);
}
export function setNotification(message, duration = 3000) {
    ui.notificationArea.textContent = message; ui.notificationArea.classList.add('show');
...
export const isTargeted = (entity, taskType = null) => state.tasks.some(t => t.target?.id === entity.id && t.status !== 'pending' && (!taskType || t.type === taskType));
